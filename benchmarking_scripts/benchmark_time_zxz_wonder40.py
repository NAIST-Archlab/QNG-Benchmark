...
for num_layers in range(1, 10):
    ...
    np.savetxt(f'./data/time/wonder40/times_{num_qubits}qubit_{num_layers}layer_{zxz_layer.__name__}.txt', np.round(np.array(times)/num_steps, 4))
    print(f'TIME_{num_qubits}qubit_{num_layers}layer_{zxz_layer.__name__}', np.round(np.array(times)/num_steps, 4))
